  ...
  <div class="channel-plugin-view">
    ...
    <div v-if="!activeChannel" class="mgmt-view channel-mgmt">
      <div class="view-header">
        <h2>企业微信频道</h2>
        <p class="view-description">管理企业微信智能机器人频道集成</p>
      </div>
      ...
      <div class="mgmt-card-grid">
        <n-card v-for="channel in channels" :key="channel.id" class="mgmt-channel-card">
          <div class="mgmt-channel-header">
            <div class="mgmt-channel-icon is-wecom">
        ...
        <n-empty v-if="channels.length === 0" description="暂无企业微信频道" style="grid-column: 1 / -1; padding: 40px 0" />
      </div>

      <n-drawer v-model:show="showAddDrawer" :width="500" placement="right">
        <n-drawer-content title="添加企业微信频道">
          <n-form :model="newChannel" label-placement="left" label-width="110">
            <n-form-item label="名称">
              <n-input v-model:value="newChannel.name" placeholder="频道名称" />
            </n-form-item>
            <n-form-item label="BotID">
              <n-input v-model:value="newChannel.config.botId" placeholder="智能机器人的 BotID" />
            </n-form-item>
            <n-form-item label="BotSecret">
              <n-input v-model:value="newChannel.config.botSecret" type="password" placeholder="长连接专用 Secret" />
            </n-form-item>
            <n-form-item label="欢迎语">
              <n-input v-model:value="newChannel.config.welcomeMessage" placeholder="用户首次进入会话时自动发送（可选）" />
            </n-form-item>
      ...
      <n-drawer v-model:show="showEditDrawer" :width="500" placement="right">
        <n-drawer-content title="编辑企业微信频道">
          <n-form :model="editingChannel" label-placement="left" label-width="110">
            ...
            <n-form-item label="BotID">
              <n-input v-model:value="editingChannel.config.botId" placeholder="智能机器人的 BotID" />
            </n-form-item>
            <n-form-item label="BotSecret">
              <n-input v-model:value="editingChannel.config.botSecret" type="password" placeholder="长连接专用 Secret" />
            </n-form-item>
            <n-form-item label="欢迎语">
              <n-input v-model:value="editingChannel.config.welcomeMessage" placeholder="用户首次进入会话时自动发送（可选）" />
            </n-form-item>
            <n-form-item label="绑定智能体">
              <n-select
    ...
    <div v-else class="channel-chat-view">
      ...
      <header class="chat-topbar">
        <div class="topbar-left">
          ...
          <div class="channel-icon wecom"><MessageSquare :size="16" /></div>
          ...
          <template v-if="activeConversation">
            <div class="content-inner">
              <div class="section-header">
                <MessageSquare :size="16" class="section-header-icon" />
...
import {
  Plus, MoreVertical, MessageSquare, ArrowLeft, Radio, Copy
...
.channel-icon.wecom { background: #07c160; color: white; }
